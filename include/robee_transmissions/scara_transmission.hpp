...
namespace robee_transmission_interface
{
  ...
  /**
   *
   * This transmission relates <b>two actuators</b> and <b>two joints</b> through
   * a scara crew mechanism, as illustrated below. \image html
   * scara_transmission.png
   *
   * <CENTER>
   * <table>
   ...
   * - \f$ j_1 is prismatic along an axis, and  \f$ j_2 is rotation round that
   * same axis, with actuator \f$ a_1 \f$ a_2 respectively.
   * - \f$ tr \f$ represents a joint transmission ratio. Reducers/amplifiers are
   * allowed (depicted as timing belts in the figure). A transmission ratio can
   * take any real value \e except zero. In particular:
   ...
   *
   * \note This implementation currently assumes a specific layout for location of
   * the actuators and joint axes which is common in robotic mechanisms. Please
   * file an enhancement ticket if your use case does not adhere to this layout.
   *
  ...
  public:
     ...
     */
    ScaraTransmission(double screw_reduction,
    ...
    void configure(
        ...
        const std::vector<transmission_interface::ActuatorHandle> &
            actuator_handles) override;

    /// Transform variables from actuator to joint space.
    /**
     * \pre Actuator and joint vectors must have size 2 and point to valid data.
     *  To call this method it is not required that all other data vectors contain
     * valid data, and can even remain empty.
     */
  ...
  inline ScaraTransmission::ScaraTransmission(
      double screw_reduction, const std::vector<double> &joint_reduction,
      const std::vector<double> &joint_ppr,
      const std::vector<double> &joint_offset)
      : screw_reduction_(screw_reduction),
        joint_reduction_(joint_reduction),
        joint_ppr_(joint_ppr),
        joint_offset_(joint_offset)
  {
    if (num_joints() != joint_reduction_.size() ||
        num_joints() != joint_offset_.size() ||
        num_joints() != joint_ppr_.size())
    {
      RCLCPP_INFO(rclcpp::get_logger("ScaraTransmission"),
                  "Reduction, ppr and offset vectors must have size 2.");
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("ScaraTransmission"),
                       "joint_ppr_[0] =" << joint_ppr_[0]);
    for (auto &x : joint_ppr_)
    {
      x /= 2 * M_PI;
    }
    RCLCPP_INFO_STREAM(rclcpp::get_logger("ScaraTransmission"),
          ...
          actuator_handles)
  {
    ...
    joint_position_last_.push_back(0.0);
    joint_position_ = get_ordered_handles(joint_handles, joint_names,
                                          hardware_interface::HW_IF_POSITION);
    joint_velocity_ = get_ordered_handles(joint_handles, joint_names,
                                          hardware_interface::HW_IF_VELOCITY);
    joint_effort_ = get_ordered_handles(joint_handles, joint_names,
                                        hardware_interface::HW_IF_EFFORT);

    if (joint_position_.size() == 0 && joint_velocity_.size() == 0 &&
        joint_effort_.size() == 0)
    {
      throw transmission_interface::Exception(
          "Not enough valid or required joint handles were presented.");
    }

    if (joint_position_.size() > 0 && joint_position_.size() != 2)
      throw transmission_interface::Exception(
          " illegal joint_position_.size() bigger then zero but not 2");
    if (joint_velocity_.size() > 0 && joint_velocity_.size() != 2)
      throw transmission_interface::Exception(
          " illegal joint_velocity_.size() bigger then zero but not 2");
    if (joint_effort_.size() > 0 && joint_effort_.size() != 2)
      throw transmission_interface::Exception(
          " illegal joint_effort_.size() bigger then zero but not 2");

    actuator_position_ = get_ordered_handles(actuator_handles, actuator_names,
                                             hardware_interface::HW_IF_POSITION);
    actuator_velocity_ = get_ordered_handles(actuator_handles, actuator_names,
                                             hardware_interface::HW_IF_VELOCITY);
    actuator_effort_ = get_ordered_handles(actuator_handles, actuator_names,
                                           hardware_interface::HW_IF_EFFORT);

    if (actuator_position_.size() == 0 && actuator_velocity_.size() == 0 &&
        actuator_effort_.size() == 0)
    {
      throw transmission_interface::Exception(
    ...
    if (actuator_position_.size() > 0 && actuator_position_.size() != 2)
      throw transmission_interface::Exception(
          " illegal actuator_position_.size() bigger then zero but not 2");
    if (actuator_velocity_.size() > 0 && actuator_velocity_.size() != 2)
      throw transmission_interface::Exception(
          " illegal actuator_velocity_.size() bigger then zero but not 2");
    if (actuator_effort_.size() > 0 && actuator_effort_.size() != 2)
      throw transmission_interface::Exception(
          " illegal actuator_effort_.size() bigger then zero but not 2");
  ...
  inline void ScaraTransmission::actuator_to_joint()
  {
    ...
    {
      auto ap0 = actuator_position_[0].get_value() /
                 (joint_ppr_[0] * joint_reduction_[0]);
      auto ap1 = actuator_position_[1].get_value() /
                 (joint_ppr_[1] * joint_reduction_[1]);
    ...
    {
      auto av0 = actuator_velocity_[0].get_value() /
                 (joint_ppr_[0] * joint_reduction_[0]);
      auto av1 = actuator_velocity_[1].get_value() /
                 (joint_ppr_[1] * joint_reduction_[1]);
    ...
    // if (debug > 50  || ((!std::isnan(joint_position_[0].get_value())) &&
    // (!std::isnan(joint_position_[1].get_value())) &&
    ...
    //     "j[0] = " <<  joint_position_[0].get_value() <<  ", a[0] = " <<
    //     actuator_position_[0].get_value() << "\n"<< "j[1] = " <<
    //     joint_position_[1].get_value() <<  ", a[1] = " <<
    //     actuator_position_[1].get_value());
  ...
  inline void ScaraTransmission::joint_to_actuator()
  {
    if (actuator_position_.size() == num_joints())
    {
      double joints_offset_applied[2] = {
          joint_position_[0].get_value() - joint_offset_[0],
          joint_position_[1].get_value() - joint_offset_[1]};

      actuator_position_[0].set_value(
          (joints_offset_applied[0] * screw_reduction_ +
           joints_offset_applied[1]) *
          joint_reduction_[0] * joint_ppr_[0]);
      actuator_position_[1].set_value(joints_offset_applied[1] *
                                      joint_reduction_[1] * joint_ppr_[1]);
    }

    if (actuator_velocity_.size() == num_joints())
    {
      actuator_velocity_[1].set_value(joint_velocity_[1].get_value() *
                                      joint_reduction_[1] * joint_ppr_[1]);

      actuator_velocity_[0].set_value(
          (joint_velocity_[0].get_value() * screw_reduction_ +
           actuator_velocity_[1].get_value()) *
          joint_reduction_[0] * joint_ppr_[0]);
    }
  ...
  std::string ScaraTransmission::get_handles_info() const
  {
    return std::string("Got the following handles:\n") + "Joint position: " +
           transmission_interface::to_string(get_names(joint_position_)) +
           ", Actuator position: " +
           transmission_interface::to_string(get_names(actuator_position_)) +
           "\n" + "Joint velocity: " +
           transmission_interface::to_string(get_names(joint_velocity_)) +
           ", Actuator velocity: " +
           transmission_interface::to_string(get_names(actuator_velocity_)) +
           "\n" + "Joint effort: " +
           transmission_interface::to_string(get_names(joint_effort_)) +
           ", Actuator effort: " +
           transmission_interface::to_string(get_names(actuator_effort_));
  }
